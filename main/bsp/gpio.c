...
status_t
gpio_init (const config_pins_t *pins, const config_general_t *gen)
{
    assert(pins);
    assert(gen);
    ...
    _set_pin_ctx(
        &_output_pins[OUTPUT_READER_BUZZER], pins->reader_buzzer, false);
    ...
    for (int i = 0; i < INPUT_INVAL; i++)
    {
        ...
        if (ctx->init)
        {
            ...
            if (err != ESP_OK)
            {
                ERROR("Can't set pin %u to input: %s",
                      ctx->pin,
                      esp_err_to_name(err));
            }
        }
    ...
    for (int i = 0; i < OUTPUT_INVAL; i++)
    {
        ...
        if (ctx->init)
        {
            ...
            if (err != ESP_OK)
            {
                ERROR("Can't set pin %u to output: %s",
                      ctx->pin,
                      esp_err_to_name(err));
            }
        }
    ...
    if (err != ESP_OK)
    {
        ERROR("Can't set aux 1 pulldown: pin %u", _input_pins[INPUT_AUX1].pin);
    }
    err = gpio_set_pull_mode(_input_pins[INPUT_AUX2].pin, GPIO_PULLDOWN_ONLY);
    if (err != ESP_OK)
    {
        ERROR("Can't set aux 2 pulldown: pin %u", _input_pins[INPUT_AUX2].pin);
    }
...
status_t
gpio_out_set (output_t out, bool state)
{
    if (out >= OUTPUT_INVAL)
    {
        return -STATUS_INVAL;
...
static void
_set_pin_ctx (pin_ctx_t *ctx, int pin, bool rev)
{
    assert(ctx);
