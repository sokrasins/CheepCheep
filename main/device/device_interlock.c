...
// When there is an active interlock session, the hardware will update the
// server with power used. This controls the frequency for the updates.
#define ILOCK_POWER_UPDATE_PERIOD 10U // s

typedef struct
{
    char id[ILOCK_SESS_ID_BYTES_MAX]; // ID associated with the session,
                                      // delivered by tßhe server
    int    kwh;
...
static void
interlock_handle_swipe (wieg_evt_t event, card_t *card, void *ctx)
{
    ...
    if (!ilock_has_session(&ilock_ctx->session))
    {
        ...
        if (STATUS_OK != status)
        {
            ERROR("Couldn't start interlock session with card %u: %d",
                  card->raw,
                  status);
            signal_alert();
        }
...
static bool
ilock_has_session (const interlock_session_t *sess)
{
    return 0 != strlen(sess->id);
}

static bool
ilock_session_match (const interlock_session_t *sess, char *id)
{
    return 0 == strcmp(id, sess->id);
}

static void
ilock_end_session (interlock_session_t *sess)
{
...
void
ilock_power_control (const config_interlock_t *config, bool on)
{
    if (on)
    {
        //
...
static void
ilock_pwr_update_cb (TimerHandle_t xTimer)
{
    // TODO check if client is connected

    if (ilock_has_session(&_ctx.session)
        && !ilock_session_match(&_ctx.session, "system"))
    {
        // TODO: get interlock power usage
        msg_t msg;
        msg.type                     = MSG_ILOCK_SESS_UPDATE;
        msg.ilock_update.session_kwh = _ctx.session.kwh;
        memcpy(msg.ilock_update.session_id,
               _ctx.session.id,
               ILOCK_SESS_ID_BYTES_MAX);
        status_t status = client_send_msg(&msg);
        if (STATUS_OK != status)
        {
            ERROR("Failed to send interlock update %s: %d",
                  _ctx.session.id,
                  status);
        }
    }
...
client_cmd_handler (msg_t *msg)
{
    ...
    if (msg->type == MSG_ILOCK_SESS_START)
    {
        ...
        memcpy(_ctx.session.id,
               msg->ilock_start_rsp.session_id,
               ILOCK_SESS_ID_BYTES_MAX);
        _ctx.session.kwh = 0;
