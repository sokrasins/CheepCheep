...
// Adapted from
// https://github.com/espressif/esp-idf/tree/v5.4.2/examples/system/ota/native_ota_example
...
status_t
ota_dfu_init (const config_dfu_t *config)
{
    _ctx.config = config;
    memset(_ctx.ota_write_data, 0, DFU_BUFFSIZE + 1);

    // Check if ota dfu is enabled. If not, return before we set the network
    // event handler.
    ...
    {
        ERROR(
            "No DFU server URL provided in the config, skipping check for new "
            "firmware");
...
status_t
ota_mark_application (bool valid)
{
...
dfu_task (void *params)
{
    ...
    {
        WARN("Configured OTA boot partition at offset 0x%08" PRIx32
    ...
    {
        int data_read
            = esp_http_client_read(client, ctx->ota_write_data, DFU_BUFFSIZE);
        if (data_read < 0)
        {
        ...
        else if (data_read > 0)
        {
            if (image_header_was_checked == false)
            {
                                    ...
                                    + sizeof(esp_app_desc_t))
                {
                    ...
                    memcpy(
                        &new_app_info,
                        &ctx->ota_write_data[sizeof(esp_image_header_t)
                                             + sizeof(
                    ...
                    if (esp_ota_get_partition_description(running,
                        ...
                        == ESP_OK)
                    {
                        INFO("Running firmware version: %s",
                    ...
                    if (esp_ota_get_partition_description(last_invalid_app,
                        ...
                        == ESP_OK)
                    {
                        INFO("Last invalid firmware version: %s",
                    ...
                    if (last_invalid_app != NULL)
                    {
                            ...
                            == 0)
                        {
                            ...
                            WARN(
                                ...
                                invalid_app_info.version);
                            WARN(
                                "The firmware has been rolled back to the "
                                "previous version.");
                            http_cleanup(client);
                            goto kill_task;
                        }
                    ...
                    {
                        if (memcmp(new_app_info.version,
                                   running_app_info.version,
                                   sizeof(new_app_info.version))
                            == 0)
                        {
                            WARN(
                                ...
                                "We will not continue the update.");
                            http_cleanup(client);
                            goto kill_task;
                        }
                    }
            ...
            err = esp_ota_write(
                update_handle, (const void *)ctx->ota_write_data, data_read);
            if (err != ESP_OK)
            {
                http_cleanup(client);
                esp_ota_abort(update_handle);
        ...
        {
            // As esp_http_client_read never returns negative error code, we
            // rely on `errno` to check for underlying transport connectivity
            // closure if any
...
_http_event_handler (esp_http_client_event_t *evt)
{
    switch (evt->event_id)
    {
        ...
        case HTTP_EVENT_ON_HEADER:
            DEBUG("HTTP_EVENT_ON_HEADER, key=%s, value=%s",
                  evt->header_key,
                  evt->header_value);
            break;
