...
static void ws_evt_cb(void            *handler_args,
...
ws_init (char *url)
{
    ...
    esp_websocket_client_config_t ws_cfg = {
        ...
        .skip_cert_common_name_check = true, // For now skip verification, but
                                             // this should change in production
        .crt_bundle_attach
        = esp_crt_bundle_attach,       // Basic cert bundle for most common
        .reconnect_timeout_ms = 10000, // Default
        .network_timeout_ms   = 10000, // Default
        .ping_interval_sec
        = 0xFFFFFFFF, // Disable the automated ping, the server expects a
                      // client-level ping-pong
    ...
    esp_err_t err = esp_websocket_register_events(
        _ctx.client, WEBSOCKET_EVENT_ANY, ws_evt_cb, (void *)&_ctx.client);
    if (err != ESP_OK)
    {
        ERROR("Unable to register websocket event handler: %s",
              esp_err_to_name(err));
        return -STATUS_IO;
    }
...
status_t
ws_evt_cb_register (ws_evt_cb_t cb, void *ctx)
{
...
ws_send (cJSON *msg)
{
    ...
    if (_ctx.connected)
    {
        ...
        {
            ERROR(
                "The client is NULL, but status indicates it's connected. "
                "Setting disconnected state");
            _ctx.connected = false;
        ...
        else
        {
            ...
            int rc = esp_websocket_client_send_text(
                _ctx.client, pkt, strlen(pkt), portMAX_DELAY);
            free(pkt);
           ...
           void            *event_data)
{
    ...
    switch (event_id)
    {
        ...
        case WEBSOCKET_EVENT_DISCONNECTED:
            ...
            {
                ERROR("reported from esp-tls: %d",
                      data->error_handle.esp_tls_last_esp_err);
                ERROR("reported from tls stack: %d",
                      data->error_handle.esp_tls_stack_err);
                ERROR("captured as transport's socket errno: %d",
        ...
        case WEBSOCKET_EVENT_DATA:
            ...
            {
                WARN("Received closed message with code=%d",
            ...
            // parsed further.
            // TODO: msg is static so that we can make sure it got deleted
            // before we allocate new json mem. I think this is not necessary.
            // Examine this with heap trace.
            if (msg != NULL)
            {
                cJSON_Delete(msg);
            }
