// Adapted from:
// https://forums.freertos.org/t/how-i-solved-the-memory-leak-problem-in-a-complex-program-with-many-tasks/23035
...
void
debug_mem_start (void)
{
    if (hPrintMem == NULL && hHeapMonitor == NULL)
    {
        xTaskCreate(f_PrintTasks,
                    "f_PrintTasks",
                    8196,
                    NULL,
                    tskIDLE_PRIORITY + 5,
                    &hPrintMem);
        xTaskCreate(f_HeapMonitor,
                    "f_HeapMonitor",
                    4096,
                    NULL,
                    tskIDLE_PRIORITY + 5,
...
void
f_startStopMemory (void)
{
    xTaskCreate(
        f_stopMemory, "f_stopMemory", 2500, NULL, tskIDLE_PRIORITY + 10, NULL);
}
...
f_PrintTasks (void *parameters)
{
    ...
    while (!StopMemory)
    {
        ...
        if (taskStatusArray != NULL)
        {
            ...
            {
                printf("| %-17s | %-9s | %-8u | %-20lu |\n",
                       taskStatusArray[i].pcTaskName,
                       (taskStatusArray[i].eCurrentState == eRunning
                            ? "Running"
                            : "Blocked"),
...
void
f_memoria (char *tarefa)
{
    INFO("Task memory usage (%s): %d bytes",
         tarefa,
         uxTaskGetStackHighWaterMark(NULL) * sizeof(StackType_t));
}

void
f_PrintHeapHistorico (uint32_t atual)
{
    ...
    {
        if (heap_history[i] < min)
        {
            min = heap_history[i];
        }
        if (heap_history[i] > max)
        {
            max = heap_history[i];
        }
    }
...
void
f_HeapMonitor (void *pvParameter)
{
    vTaskDelay(
        pdMS_TO_TICKS(60000)); // wait 1 minute before starting heap trace
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (rec->address == NULL || rec->size == 0)
        {
            continue;
        }
