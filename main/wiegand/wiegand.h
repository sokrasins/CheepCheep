...
{
    uint32_t
        raw; // Unparsed card data. This is how membermatters reports cards.
    struct __attribute__((packed))
    {
        uint16_t user_id;
        uint16_t facility; // If 24-bit mode is selected, this value won't
                           // exceed 0xFF
    };
} card_t;
 ...
 * @param d0 GPIO number (not physical pin number) of the d0 signal. This pin
 * does not have to be configured.
 * @param d1 GPIO number (not physical pin number) of the d1 signal. This pin
 * does not have to be configured.
 ...
 */
wieg_evt_handle_t wieg_evt_handler_reg(wieg_evt_t    event,
                                       wieg_evt_cb_t cb,
                                       void         *ctx);
